...
int editor_run(const EditorOptions &opts, const EditorHost *host) {
  ...
  while (running) {
    ...
    // basiyordu. Olculdu (gecici sonda): bu noktada CurrentWindowStack.Size == 1,
    // yani acik KULLANICI penceresi yok -- End() eslesmemis. Kaynagi PR #7 degil,
    // main'in kendisi: main'de de editor_app.cpp 8 `ImGui::Begin(` ve 9
    // `ImGui::End();` tasiyor. Ustteki panellerin (Dunya, Kaynaklar, Materyal
    // Graph) hepsi kendi End'ini zaten cagiriyor; fazlalik silindi.
